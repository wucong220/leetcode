...
public class CN743 {
    class Solution {
        public int networkDelayTime(int[][] times, int n, int k) {
            ...
            cost[k] = 0;
            TreeSet<Integer> pq = new TreeSet<>(Comparator.comparingInt(a->cost[(int)a]).thenComparing(a->(int)a));
            pq.add(k);
            while(!pq.isEmpty()){
                Integer curr = pq.pollFirst();
                for (int[] time : times) {
                    final int src = time[0]-1;
                    final int des = time[1]-1;
                    final int timeCost = time[2];
                    if(src ==curr){
                        if(cost[curr]+ timeCost <cost[des]){
                            pq.remove(des);
                            cost[des] = cost[curr] + timeCost;
                            pq.add(des);
                            pre[des] = curr;
                        }
                    }
